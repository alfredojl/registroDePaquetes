    ...
    .then(async () => {
      ...
      for (file of archs) {
        ...
        let a = await conn.query(
          `SELECT Id FROM T15 WHERE C22 = ${folio} AND C25 ${
            tomo ? "= " + tomo : "IS NULL"
          } AND C11242 = ${nh}`
        );
        Id = a[0] ? a[0].Id : null;
        ...
        if (!Id) {
          console.log(
            `${file}: \t\t\t${folio} ${
              tomo ? "Tomo " + tomo : ""
            }\t${nh} no está en SICE. ----> Error\t${++index} de ${
              archs.length
            }`.red
          );
          fs.appendFileSync(
          ...
          console.log(
            `${file}: \t\t${folio} ${
              tomo ? "Tomo " + tomo : ""
            }\t${nh} sin número de imágenes. ----> Error\t${++index} de ${
              archs.length
            }`.red
          );
          fs.appendFileSync(
          ...
          console.log(
            `Eliminando ${Id.toString()}.pdf\t[ ${moment().format(
              "L HH:mm:ss"
            )} ]`
          );
          await sftp
            ...
            .then(async () => {
              ...
              return await sftp.put(
                path.join(ruta, file),
                path.join(pathSFTP, Id.toString() + ".pdf")
              );
            })
            .then(() => {
              // fs.renameSync()
