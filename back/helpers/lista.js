...
const getList = async () => {
    try {
        ...
        let ruta = path.join(os.homedir(), 'SICE', 'nuevoRoot', 'gerardo');
        let folio = null, tomo = null, direc = null, base = null, sep = null, nh = null;
        ...
        for (file of dirs) {
            folio = null; tomo = null; sep = null; nh = null;
            ...
            if (f.length == 0) {
                console.log("*******************Creando***********************");
                let m = await axios.get('http://digitalizacion.pjcdmx.gob.mx/consulta_folio.php', {
                    params: {
                        f: folio
                    }
                });
                ...
                if (m.encontrado) {
                    ...
                    fs.appendFileSync(path.resolve('foliosASubir.csv'), `${folio},${tomo},${nh},notFinded\n`, 'utf-8')
                }
            } else {
                if (f[0].procesado)
                    fs.appendFileSync(path.resolve('foliosASubir.csv'), `${folio},${tomo},${nh},procesado\n`, 'utf-8')
                else if (f[0].validado) {
                    let folioUpdated = await Folio.updateOne({ folio, tomo }, { numeroImagenes: nh });
                    fs.appendFileSync(path.resolve('foliosASubir.csv'), `${folio},${tomo},${nh},validado\n`, 'utf-8')
                }
                else {
                    ...
                    if (m.encontrado) {
                        ...
                        fs.appendFileSync(path.resolve('foliosASubir.csv'), `${folio},${tomo},${nh},updated\n`, 'utf-8')
                    }
                    else {
                        fs.appendFileSync(path.resolve('foliosASubir.csv'), `${folio},${tomo},${nh},notUpdated\n`, 'utf-8')
                    }
                }
            }
        }
